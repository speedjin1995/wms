...
<div class="modal fade" id="integrationModal" tabindex="-1">
  <div class="modal-dialog">
    <div class="modal-content custom-model-content-box">
      ...
      <div class="modal-footer custom-model-fotter-box">
        <button type="button" class="btn custom-close-btn btn-sm" data-dismiss="modal"><?=$languageArray['cancel_code'][$language]?></button>
        <button type="button" class="btn custom-save-btn btn-sm" id="integrationExportBtn"><?=$languageArray['export_code'][$language]?></button>
      </div>
    </div>
  </div>
</div>
  ...
  var table = $("#weightTable").DataTable({
    ...
    "footerCallback": function(row, data, start, end, display) {
      ...
      if (allowPrice == 'Y') {
        ...
        $(api.column(12).footer()).html(Object.entries(totalReject).map(function(e) { return e[0] + ' ' + e[1].toFixed(2); }).join('<br>') || '0.00');
      } else {
        totalReject = api
          .column(12, { page: 'current' })
          .data()
