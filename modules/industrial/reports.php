...
if(!isset($_SESSION['userID'])){
  ...
  if ($allowIntegration == 'Y') {
    $intStmt = $db->prepare("SELECT id, integration_type, status, name, config_json FROM integration_configs WHERE company_id = ? AND module = 'industrial' AND deleted = 0");
    ...
    <div class="card results-card show-dt-controls">
      ...
      <div class="card-body">
        <table id="weightTable" class="table data-table">
          ...
          <tfoot>
            <tr>
              <th colspan="7"><?=$languageArray['total_code'][$language]?></th>
              <th></th>
              <th></th>
              <th></th>
              <th></th>
...
$(function () {
  ...
  if (allowIntegration === 'Y') {
    $('#exportIntegration').on('click', function() {
      var statusFilter = $('#transactionStatusFilter').val();
...
function initTable() {
  return $("#weightTable").DataTable({
    ...
    footerCallback: function(row, data, start, end, display) {
      var api = this.api();
      var totalItem = api.column(7, { page: 'current' }).data().reduce(function(a, b) { 
        return a + parseFloat(String(b || 0).replace(/,/g, '')); 
      }, 0);
      var totalGross = api.column(8, { page: 'current' }).data().reduce(function(a, b) { 
        return a + parseFloat(String(b || 0).replace(/,/g, '')); 
      }, 0);
      var totalTare = api.column(9, { page: 'current' }).data().reduce(function(a, b) { 
        return a + parseFloat(String(b || 0).replace(/,/g, '')); 
      }, 0);
      var totalNett = api.column(10, { page: 'current' }).data().reduce(function(a, b) { 
        return a + parseFloat(String(b || 0).replace(/,/g, '')); 
      }, 0);
      var totalVariance = api.column(11, { page: 'current' }).data().reduce(function(a, b) { 
        return a + parseFloat(String(b || 0).replace(/,/g, '')); 
      }, 0);

      $(api.column(7).footer()).html(totalItem);
      $(api.column(8).footer()).html(totalGross.toFixed(2));
      $(api.column(9).footer()).html(totalTare.toFixed(2));
      $(api.column(10).footer()).html(totalNett.toFixed(2));
      $(api.column(11).footer()).html(totalVariance.toFixed(2));
    }
  });
}
</script>
