...
if(isset($post['staffName'], $post['customer'])){
	...
	if(isset($post['userId']) && $post['userId'] != null && $post['userId'] != ''){
	    if ($update_stmt = $db->prepare("UPDATE destinations SET destination_name = ? WHERE id = ?")) {
	    ...
	    if ($insert_stmt = $db->prepare("INSERT INTO destinations (destination_name, customer) VALUES (?, ?)")){	
