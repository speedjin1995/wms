...
if (!empty($data)) {
    foreach ($data as $rows) {
        ...
        $deleted = "0";
        $unitQuery = "SELECT * FROM grades WHERE units = '".mysqli_real_escape_string($db, $Unit)."'  AND customer = '".mysqli_real_escape_string($db, $company)."' AND deleted = '".mysqli_real_escape_string($db, $deleted)."'";
        ...
        if(empty($unitRow)){
            ...
            $errMsg = "Grade: ".$Unit." already exists.";
            $errorArray[] = $errMsg;
            continue;
        }
