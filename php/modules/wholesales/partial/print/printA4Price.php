...
foreach ($groups as $g) {
    ...
    if (!empty($withDetails) && $withDetails == 'Y') {
        $colSpan = $includePrice ? 8 : 6;
        $parts = [];
        foreach ($g['rows'] as $r) {
            $parts[] = number_format($r['net'],2).$r['unit'];
        }
        $rows .= '<tr class="detail-row">';
        $rows .= '<td></td>';
        $rows .= '<td colspan="'.$colSpan.'" class="detail-text">'.implode(' / ', $parts).'</td>';
        $rows .= '</tr>';
    }
...
<head>
    ...
    <style>
        * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
        ...
        .info-row { display: flex; margin-bottom: 1px; font-size: 12px; }
