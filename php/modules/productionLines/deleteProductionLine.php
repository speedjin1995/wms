...
if(isset($_POST['userID'])){
	...
	if ($type == 'MULTI'){
		...
		if ($stmt2 = $db->prepare("UPDATE production_lines SET deleted=?, modified_by=? WHERE id IN ($ids)")) {
		...
		if ($stmt2 = $db->prepare("UPDATE production_lines SET deleted=?, modified_by=? WHERE id=?")) {
