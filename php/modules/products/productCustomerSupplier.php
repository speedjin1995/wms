...
if (isset($_POST['product_id']) && $_POST['product_id'] != '') {
    ...
    if (isset($_POST['no']) && count($_POST['no']) > 0) {
        ...
        $customerType = isset($_POST['customerType']) ? (array)$_POST['customerType'] : [];
        ...
        foreach ($no as $key => $number) {
            ...
            $cType = isset($customerType[$key]) && $customerType[$key] != '' ? $customerType[$key] : 'Local';
            ...
            if (isset($customerProductId[$key]) && $customerProductId[$key] != '') {
                ...
                if ($stmt = $db->prepare("UPDATE product_customers SET product_id=?, customer_id=?, grade_id=?, type=?, pricing_type=?, pricing_currency=?, price=?, deleted='0', modified_by=? WHERE id=?")) {
                    $stmt->bind_param('sssssssss', $productId, $customerId, $cGradeId, $cType, $cPricingType, $cCurrency, $cPrice, $userID, $cProductId);
                    $stmt->execute();
                    $stmt->close();
                }
            } else {
                if ($stmt = $db->prepare("INSERT INTO product_customers (product_id, customer_id, grade_id, type, pricing_type, pricing_currency, price, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")) {
                    $stmt->bind_param('ssssssss', $productId, $customerId, $cGradeId, $cType, $cPricingType, $cCurrency, $cPrice, $userID);
                    $stmt->execute();
                    $stmt->close();
                }
            }
    ...
    if (isset($_POST['supplierNo']) && count($_POST['supplierNo']) > 0) {
        ...
        $supplierType = isset($_POST['supplierType']) ? (array)$_POST['supplierType'] : [];
        ...
        foreach ($supplierNo as $key => $number) {
            ...
            $sType = isset($supplierType[$key]) && $supplierType[$key] != '' ? $supplierType[$key] : 'Local';
            ...
            if (isset($supplierProductId[$key]) && $supplierProductId[$key] != '') {
                ...
                if ($stmt = $db->prepare("UPDATE product_suppliers SET product_id=?, supplier_id=?, grade_id=?, type=?, purchasing_pricing_type=?, purchasing_pricing_currency=?, purchasing_price=?, deleted='0', modified_by=? WHERE id=?")) {
                    $stmt->bind_param('sssssssss', $productId, $supplierId, $sGradeId, $sType, $sPricingType, $sCurrency, $sPrice, $userID, $sProductId);
                    $stmt->execute();
                    $stmt->close();
                }
            } else {
                if ($stmt = $db->prepare("INSERT INTO product_suppliers (product_id, supplier_id, grade_id, type, purchasing_pricing_type, purchasing_pricing_currency, purchasing_price, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")) {
                    $stmt->bind_param('ssssssss', $productId, $supplierId, $sGradeId, $sType, $sPricingType, $sCurrency, $sPrice, $userID);
                    $stmt->execute();
                    $stmt->close();
                }
            }
