...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    // Load statuses for this module + Supplier entity type
    $statuses = [];
    $stmt = $db->prepare("SELECT id, status, prefix FROM statuses WHERE module = ? AND entity_type = 'Supplier'");
    ...
    // Load invoice_code from supplies table
    $supplierRow = ['invoice_code' => ''];
    $stmt = $db->prepare("SELECT invoice_code FROM supplies WHERE id = ? AND customer = ?");
    $stmt->bind_param('ss', $entityId, $company);
    $stmt->execute();
    $supplierRow = $stmt->get_result()->fetch_assoc() ?: $supplierRow;
    $stmt->close();
    ...
    echo json_encode(['status' => 'success', 'data' => $statuses, 'invoice_code' => $supplierRow['invoice_code'] ?? '']);
    ...
    // Update invoice_code on supplies table
    $stmt = $db->prepare("UPDATE supplies SET invoice_code = ? WHERE id = ? AND customer = ?");
