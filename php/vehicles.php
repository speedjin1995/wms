...
if(isset($_POST['vehicleNumber'], $_POST['driver'], $_POST['company'])){
    ...
    if($_POST['id'] != null && $_POST['id'] != ''){
        if ($update_stmt = $db->prepare("UPDATE vehicles SET veh_number=?, driver=?, attandence_1=?, attandence_2=? WHERE id=?")) {
            $update_stmt->bind_param('sssss', $vehicleNumber, $driver, $attendence1, $attendence2, $_POST['id']);
        ...
        if ($insert_stmt = $db->prepare("INSERT INTO vehicles (veh_number, driver, attandence_1, attandence_2, customer) VALUES (?, ?, ?, ?, ?)")) {
            $insert_stmt->bind_param('sssss', $vehicleNumber, $driver, $attendence1, $attendence2, $company);
