...
$id = $_POST['id'] ?? $_GET['id'];
$stmt = $db->prepare("SELECT * FROM wholesales LEFT JOIN companies ON wholesales.company = companies.id WHERE wholesales.id = ?");
...
// Function to arrange JSON data by grade
...
<html>
<head>
    <title><?php echo ($wholesale['status'] == 'DISPATCH' ? 'Dispatch' : 'Receiving') ?> Report Weighing</title>
...
<body>
    <div class="running-header">
        <div class="mb-1">
            ...
            <div class="address"><?= htmlspecialchars($wholesale['address']) ?> <?= htmlspecialchars($wholesale['address2']) ?></div>
            <div class="address"><?= htmlspecialchars($wholesale['address3']) ?> <?= htmlspecialchars($wholesale['address4']) ?></div>
            <hr>
        </div>

        <div class="d-flex justify-content-between align-items-center mb-1">
            <div class="title font-weight-bold"><?php echo ($wholesale['status'] == 'DISPATCH' ? 'Dispatch' : 'Receiving') ?> REPORT WEIGHING</div>
        ...
        <div class="grade-section page-content">
            <?php
                ...
                for($row = 0; $row < $rowsNeeded; $row++) {
                    ...
                    if($row > 0 && $row % 2 == 0) {
                        echo '<div class="row mb-3 page-break">';
                    } else {
                    ...
                    for($col = 0; $col < 3; $col++) {
                        ...
                        if($gradeIndex < $totalGrades) {
                            ...
                            echo '<tr style="font-weight: bold; background-color: #f0f0f0;"><td colspan="4">GRADE : ' . $grade . '</td></tr>';
                            echo '<tr><th>No</th><th>Gross Weight</th><th>Tare Weight</th><th>Net Weight</th></tr>';
                            ...
                            for($i = 0; $i < 10; $i++) {
                                ...
                                echo '<tr>';
                                echo '<td>' . ($i + 1) . '</td>';
                                echo '<td>' . number_format($gross, 1) . ' kg</td>';
                                echo '<td>' . number_format($tare, 1) . ' kg</td>';
                                echo '<td>' . number_format($net, 1) . ' kg</td>';
                                echo '</tr>';
                            }
                            
                            echo '<tr style="font-weight: bold;">';
                            echo '<td style="border-right: none;">T</td>';
                            echo '<td style="border-left: none; border-right: none;">' . number_format($totalGross, 1) . ' kg</td>';
                            echo '<td style="border-left: none; border-right: none;">' . number_format($totalTare, 1) . ' kg</td>';
                            echo '<td style="border-left: none;">' . number_format($totalNet, 1) . ' kg</td>';
                            echo '</tr>';
                            echo '<tr>';
                            echo '<td colspan="2">Price /kg</td>';
                            // echo '<td>RM ' . number_format($totalPrice, 2) . '</td>';
                            echo '<td colspan="2">RM ' . number_format($totalPrice, 2) . '</td>';
                            echo '</tr>';
                            echo '</table>';
                            echo '</div>';
                        }
