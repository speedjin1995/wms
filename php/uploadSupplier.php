...
if (!empty($data)) {
    foreach ($data as $rows) {
        ...
        $deleted = "0";
        $unitQuery = "SELECT * FROM supplies WHERE supplier_name = '".mysqli_real_escape_string($db, $SupplierName)."' AND customer = '".mysqli_real_escape_string($db, $company)."' AND deleted = '".mysqli_real_escape_string($db, $deleted)."'";
        $unitDetail = mysqli_query($db, $unitQuery);
        $unitRow = mysqli_fetch_assoc($unitDetail);

        if(empty($unitRow)){
            if ($insert_stmt = $db->prepare("INSERT INTO supplies (parent, supplier_code, reg_no, supplier_name, supplier_address, supplier_address2, supplier_address3, supplier_address4, states, billing_name, billing_address, billing_address2, billing_address3, billing_address4, billing_state, supplier_phone, pic, fax, customer) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
                $insert_stmt->bind_param('sssssssssssssssssss', $Parent, $SupplierCode, $RegistrationNo, $SupplierName, $Address, $Address2, $Address3, $Address4, $State, $BillingName, $BillingAddress, $BillingAddress2, $BillingAddress3, $BillingAddress4, $BillingState, $Phone, $PIC, $Fax, $company);
                $insert_stmt->execute();
