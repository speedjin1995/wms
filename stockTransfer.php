...
<div class="content-header custom-title-content-box">
  <div class="container-fluid">
    <div class="row">
      <div class="col-sm-6">
        <h1 class="custom-title"><?=$languageArray['stock_transfer_code'][$language]?></h1>
      </div>
    </div>
  </div>
</div>
    ...
    <div class="modal-content custom-model-content-box">
      <form role="form" id="transferForm">
        <div class="modal-header custom-model-header-box">
          <h4 class="modal-title custom-model-title-txt"><?=$languageArray['stock_transfer_code'][$language]?></h4>
        ...
        <div class="modal-body custom-model-body-box">
          ...
          <div class="row mb-3">
            <div class="col-md-12">
              <div class="form-group">
                ...
                <textarea class="form-control custom-remarks-txtarea" id="transferRemarks" name="remarks" rows="2"></textarea>
              </div>
            </div>
          </div>
          ...
          <div class="row">
            ...
            <div class="col-md-6">
              <div class="custom-card-box custom-card-product-info-box">
                <div class="card-header custom-card-box-header">
                  <h6 class="custom-card-box-header-title"><?=$languageArray['batch_code'][$language]?> A: <span id="batchALabel"></span>
            ...
            <div class="col-md-6">
              <div class="custom-card-box custom-card-range-box">
                <div class="card-header custom-card-box-header">
                  <h6 class="custom-card-box-header-title"><?=$languageArray['batch_code'][$language]?> B: <span id="batchBLabel"></span>
          ...
          <span class="batch-txt-notice"><i class="fas fa-info-circle"></i> <?=$languageArray['drag_rows_hint_code'][$language]?></span>
        </div>
        ...
        <div class="modal-footer custom-model-fotter-box">
          ...
          <button type="submit" class="btn custom-save-btn" id="saveTransferBtn"><?=$languageArray['save_code'][$language]?></button>
        </div>
      </form>
    </div>
  </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content custom-model-content-box">
      <form role="form" id="cancelForm">
        <div class="modal-header custom-model-header-box">
          <h4 class="modal-title custom-model-title-txt"><i class="fas fa-undo"></i> <?=$languageArray['undo_stock_transfer_code'][$language]?></h4>
...
<script>
    ...
    var table = $("#transferTable").DataTable({
      ...
      'columns': [
        ...
        { data: 'id', class: 'action-button', render: function(data) {
            var btn = '<div class="row custom-tbl-btn-icon">';
            <?php if($allowDelete == 'Y'){ ?>
              btn += '<button type="button" onclick="deactivate('+data+')" class="btn custom-reject-btn-icon btn-sm"><i class="fas fa-undo"></i></button>';
            <?php } ?>
            btn += '</div>';
            return btn;
          }
  ...
  function buildTransferRow(item, side) {
    var arrow = side === 'A'
      ? '<button type="button" class="btn btn-sm custom-reject-btn-icon btn-transfer" onclick="moveRow(this, \'B\')"><i class="fas fa-arrow-right"></i></button>'
      : '<button type="button" class="btn btn-sm custom-reject-btn-icon btn-transfer" onclick="moveRow(this, \'A\')"><i class="fas fa-arrow-left"></i></button>';
